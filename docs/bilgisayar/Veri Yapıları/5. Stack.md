---
title: Stack (Yığıt)
description: Yığıt (stack) veri yapısı, verilerin düzenli bir şekilde saklanmasını ve yönetilmesini sağlar. Son giren ilk çıkar (LIFO) mantığına dayalı olarak çalışan yığıt, birçok alanda yaygın bir şekilde kullanılmaktadır.
keywords: [yığıt, stack, LIFO, bellek yönetimi, programlama dilleri, algoritmalar, bağlı listeler]
---
...
:::tip
`push` fonksiyonu, yeni bir düğüm oluşturur ve yığıtın en üstüne ekler. `pop` fonksiyonu, yığtın en üstündeki elemanı çıkarır ve geri döndürür. `peek` fonksiyonu, yığının en üstündeki elemana bakar ve bu elemanın değerini geri döndürür.
:::
