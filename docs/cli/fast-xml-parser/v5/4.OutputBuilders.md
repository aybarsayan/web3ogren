---
description: Bu doküman, üç farklı çıktı yapılandırıcısının kullanımını ve örneklerini açıklamaktadır. JsObjBuilder, JsArrBuilder ve JsMinArrBuilder hakkında bilgi verir.
keywords: [JsObjBuilder, JsArrBuilder, JsMinArrBuilder, çıktı yapılandırıcıları, JSON, SOAP]
---

### Çıktı Yapılandırıcıları

Verilen çıktı yapılarını veya kendi çıktı yapılandırıcınızı kullanabilirsiniz.

> **Not:** Her bir çıktı yapılandırıcısının amacı, verileri belirli bir formatta düzenlemektir. Aşağıda farklı çıktı yapılandırıcılarının örnekleri yer almaktadır.
